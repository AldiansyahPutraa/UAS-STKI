...
$result=mysqli_query($koneksi,$query);
  ...
  // tampilkan kotak pencarian dan jumlah hasil pencarian
    ...
    echo "<br /><br><table><tr><td></td><td><form action='SSTANIMOTO3.php' method='GET'><input type='text' onclick=value='' size='50' name='keyword' value='$keyword' style='width: 500px; height: 30px; font-size: 16px;'> <input type='submit' value='Search'></form></td></tr></table>";
...
$query="SELECT * FROM (SELECT g.id id,kata, A, jmlkatakunci, B, jmlkatadok, jmlkatadok-A C, A/(A+B)+(A+(jmlkatadok-A))- A STANIMOTO3 FROM (SELECT *,jmlkatakunci-A B FROM (SELECT y.id,y.kata,y.freq A FROM (SELECT * FROM katakunci) AS x JOIN (SELECT * FROM token WHERE id!='0' ORDER BY id+0) as Y ON x.kata=y.kata) AS d JOIN (SELECT COUNT(kata) jmlkatakunci FROM katakunci) AS e) AS f JOIN (SELECT id,COUNT(kata) jmlkatadok FROM token GROUP BY id) AS g ON f.id=g.id GROUP BY g.id ORDER BY STANIMOTO3 DESC) AS r JOIN (SELECT * FROM berita) AS s ON r.id=s.id"; 
$result=mysqli_query($koneksi,$query);
